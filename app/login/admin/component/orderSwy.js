...
export default class OrderComponent extends Component {
	handleWeChatPay(){
		return ()=> {
            ...
            setMessage({
                type: 'app',
                title: '微信支付',
                content: 'app:apps/login/admin/component/weChatPay?fromSwy=true',
                okText: '更换其他支付方式',
	...
	handlePaySuccess(){
		return ()=> {
            ...
            setMessage({
                type: 'app',
                title: '微信支付',
                content: 'app:apps/login/admin/component/paySuccess?fromSwy=true',
                okText: '查看订单',
	...
	handleNowPay() {		
		let {clearMessage, setMessage} = this.props,
            getterByField = this.props.payload.getIn(['utils', 'getterByField']),
			id = getterByField('orderSwy.id')
        this.props.onRedirect('apps/login/orderSwy?step=2&id='+id,true)
	}
	handleRePurchase(){
		let {clearMessage, setMessage} = this.props,
            getterByField = this.props.payload.getIn(['utils', 'getterByField'])
        this.props.onRedirect('apps/login/orderSwy',true)
	}
	...
	render(){
		let message = this.props.payload.getIn(['global', 'message']),
			...
			order = getterByField('orderSwy').toJS(),
						...
						<div className={`${prefixCls}-${orderStr}-info`}>
							<div className={`${prefixCls}-order-info-order`}>
								<div className={`${prefixCls}-order-info-order-first`}>
									...
									<p style={{display: 'none'}} className={`${prefixCls}-order-info-order-first-type`}><span>订单类型：</span><span>{order.orderType == 1 ? '新购' : '续购'}订单</span></p>
									...
									<p className={`${prefixCls}-order-info-order-first-org`}><span>组织名称：</span><span>{order.orgName}</span></p>
								</div>
