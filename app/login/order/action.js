...
export function initStepSecond(id, getQrCode, onRedirect,productId) {
    return injectFuns => {
        ...
        if(productId){//个人代账  fsk
            // setTimeout(()=>{openPaySuccess(onRedirect,true)(injectFuns)},1000)
            ...
            webapi.order.queryOnlineSpOrderDetail(injectFuns.post, { 'id': id }).then(data => {
                ...
                if (data) {
                    if (!da.handleWebApiInfo(data)(injectFuns)) return
                    // injectFuns.reduce('initStepSecond',secondData,data.value)
                    initData = data.value
...
export function setInitData(curOrgId,productId) {
    return injectFuns => {
        ...
        if(productId){// 个人代账
            ...
            let isIts = location.href.indexOf('?h=its') != -1,
                params = { orgId: curOrgId }
            if(isIts){
                params.productId = 5
            }
            webapi.order.onlineBuy(post, params).then(data => {
                if(!data.result&&data.value) {
                    return da.setMessage({
                        type: 'warning',
                        title: '提示',
                ...
                if (data) {
                    ...
                    // injectFuns.reduce('appInfo',data.value)
                    if (curOrgId) {
                        webapi.order.existsUnpaidOrder(injectFuns.post, { 'orgId': curOrgId }).then(data2 => {
...
export function submitOrder(getQrCode, onRedirect) {
    return injectFuns => {
        ...
        // webapi.order.createOnlineOrder(post, list).then(data => {
        //     if (!data.result) {
        //         return da.setMessage({
        //             type: 'warning',
        //             content: data.value,
        ...
        //     return webapi.order.getWeixinPayCode(injectFuns.post, list2)
        // }).then(data => {
        //     if (data) {
        //         if(!data.result&&data.value) {
        //             return da.setMessage({
        ...
        //         }
        //         if (!da.handleWebApiInfo(data)(injectFuns)) return
        //         injectFuns.reduce('setStepSecond', orderInfo, data.value)
        //         setTimeout(function () {
        //             getQrCode(data.value.payCode)
...
export function submitSpOrder(getQrCode, onRedirect,getParam) {
    return injectFuns=>{
	    ...
	    // "orgId":3268483005678592,
	    // "productId":1
        // "beginDate":"2018-01-20", ---开始时间
	    // "endDate":"2019-01-19", ---结束时间
	    // "productPrice":2400,---产品价格
	    // "amount":2400,---成交价格
	    // "saleAmount":0,---优惠金额
	    // "timespan":1,---时长（年）
	    // "purchaseNum":10,---已购数量
	    // "addPurchaseNum":0,---新增数量
