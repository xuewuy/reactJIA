...
export function addChildRow(state, path, value) {
    let parsedPath = parsePath(path),
        bindField,rowIndex =0
    path = state.getIn(['parsedMeta', parsedPath.path])
    rowIndex = parsedPath.vars[0] || 0

    //state = state.setIn(['data_runtime'].concat('form.parentRowIndex'), rowIndex)
    state = state.updateIn((`data_runtime.form.${rowIndex}.detail`).split('.'), x => x.push(value))
    return state
}


export function delChildRow(state, path, findIndexFunc) {
    let parsedPath = parsePath(path),
        bindField
    path = state.getIn(['parsedMeta', parsedPath.path])
    bindField = state.getIn((`${path}.bindField`).split('.'))
    ...
    state = state.updateIn(`data_runtime.form.${findIndexFunc}.detail`.split('.'), x => {
        if (delRowIndex >= 0) {
...
function updateStatus(state, path, option) {
    ...
    if (option === 'delete') {
        let status = state.getIn((`data_runtime.${path}._status`).split('.'))
    ...
    if (value instanceof Map) {
        ...
        value.keySeq().forEach(k => {
            ...
            if (deletes) {
                ...
                deletes.forEach(d => {
                    result[k].push({ id: d.get('id'), _status: 3 })
                })
            }
        })
